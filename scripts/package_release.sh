...
ARCHIVE_NAME="Codex-Draft-Inbox-v$VERSION-macos-universal.zip"
ARCHIVE="$OUTPUT_ROOT/$ARCHIVE_NAME"
PACKAGE_DIR=$(mktemp -d)
STAGING="$PACKAGE_DIR/Codex Draft Inbox v$VERSION"
trap 'rm -rf -- "$PACKAGE_DIR"' EXIT

APP_PATH=$(CODEX_DRAFT_INBOX_SKIP_BRAND_ASSETS=1 CODEX_DRAFT_INBOX_UNIVERSAL=1 "$PLUGIN_ROOT/scripts/build_macos_app.sh")
...
install -m 755 "$PLUGIN_ROOT/scripts/configure_launch_agent.sh" "$STAGING/scripts/configure_launch_agent.sh"
...
(cd "$OUTPUT_ROOT" && shasum -a 256 "$ARCHIVE_NAME" > "$ARCHIVE_NAME.sha256")
