...
- 界面默认跟随 macOS 系统语言，也可以在面板中切换简体中文或 English；系统通知会使用同一种语言。
- 登录 macOS 后自动启动。
...
从 v0.3.3 开始，点击“打开任务 / 恢复会话”并成功发起跳转后，待办弹窗会立即收起；如果打开失败，弹窗会保留并显示错误，方便重试。

从 v0.4.0 开始，菜单栏 App 支持跟随系统、简体中文和 English 三种语言设置。语言选择保存在本机并立即生效，覆盖面板、状态、错误提示、辅助功能标签和系统通知；会话标题与草稿原文保持原样。
